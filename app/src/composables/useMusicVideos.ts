...
export function useMusicVideos() {
  ...
  const retrieveMetadata = useMutation({
    mutationFn: (url: string) => retrieveVideoMetadata(token.value!, url),
  })
  ...
  return { query, create, update, remove, previewFetch, retrieveMetadata, syncViews }
}
