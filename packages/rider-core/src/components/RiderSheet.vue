...
/**
 * The printed tech-rider sheet, rendered identically wherever it appears.
 *
 * Presentation only — it takes an already-loaded rider and never fetches. The
 * admin hands it the live rider by id; the public site hands it a published
 * snapshot fetched by token. Keeping the fetch outside is what lets one
 * component serve both, and keeping the resolver shared is what stops the
 * band's copy and the venue's copy from numbering channels differently.
 *
 * Deliberately styled with fixed values rather than the public site's design
 * tokens: this is a document a venue prints, and it must come out black on
 * white whatever theme the surrounding site is wearing. That is also why it
 * lives here and not in web/src, where the token lint (correctly) rejects raw
 * colours.
 */
...
const logoUrl = computed(() => props.logoUrl ?? null)
