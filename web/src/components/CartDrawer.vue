...
<template>
  <Transition name="drawer">
    <div v-if="open" class="fixed inset-0 z-50 flex justify-end">
      <!-- Backdrop -->
      <div class="absolute inset-0 bg-inverse/60" @click="close" />
        ...
        <div class="flex-1 overflow-y-auto px-5 py-4 space-y-4">
          ...
          <div v-for="item in itemList" :key="`${item.shopItemId}-${item.variantId}`" class="flex gap-3">
            ...
            <div class="flex-1 min-w-0">
              <p class="text-sm font-medium text-body line-clamp-1">{{ item.name }}</p>
              <p v-if="item.variant" class="text-xs text-muted">{{ item.variant }}</p>
