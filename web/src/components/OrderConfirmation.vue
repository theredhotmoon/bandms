...
/** Labels from the Merch module's copy — see lib/copy.ts orderCopy(). */
...
defineProps<{ copy: OrderCopy; merchHref: string }>()
...
<template>
  <div class="rounded-card border border-border bg-surface p-8 sm:p-10">
    <p v-if="status === 'loading'" class="text-muted">{{ copy.loading }}</p>
    ...
    <template v-else>
      <div class="mb-6 flex items-center gap-3">
        <span
          ...
          aria-hidden="true"
        >✓</span>
        <h1 class="text-2xl font-black text-body sm:text-3xl">{{ copy.title }}</h1>
      </div>

      <p v-if="status !== 'found'" class="text-muted">{{ copy.unknown }}</p>

      <template v-else-if="order">
        <p class="text-muted">{{ copy.thanks }}</p>

        <p class="mt-6 text-sm text-muted">
          {{ copy.orderNumber }} <span class="font-mono text-body">{{ order.uuid }}</span>
        </p>
        ...
        <ul v-if="order.items?.length" class="mt-4 divide-y divide-border border-t border-border">
          ...
          >
            ...
            <span v-if="item.ticket_uuids?.length" class="w-full pt-2">
              ...
              >{{ copy.downloadTicket }}</a>
            </span>
          </li>
        </ul>

        <p class="mt-4 border-t border-border pt-4 text-right text-sm text-body">
          {{ copy.total }} <span class="font-bold">{{ money(order.total, order.currency) }}</span>
        </p>
      </template>

      <a :href="merchHref" class="mt-8 inline-block text-sm text-accent hover:underline">{{ copy.backToMerch }}</a>
    </template>
  </div>
</template>
