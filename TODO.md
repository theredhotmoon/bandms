...
### Leftover `.worktrees/admin-rebuild-guard` directory won't delete
PR #99 (admin rebuild guard) merged and the git worktree is unregistered (`git worktree list` no longer shows it), but the physical directory at `.worktrees/admin-rebuild-guard` still won't delete — `rm -rf`/`Remove-Item` both fail with "device or resource busy" on `.../app`. Tried and ruled out: killed the one Node process (a leftover Vite dev server) whose command line referenced the path; ran `docker compose down` on the `bandms_*` stack that had been started from that worktree's `docker-compose.yml` (`working_dir` pointed there) — neither released the lock. Matches this repo's own documented WSL2 file-sharing quirks; the fix is almost certainly `wsl --shutdown` (restarts the WSL VM), not attempted because it would also bounce every other Docker-based project currently running (`movesetter-app`, `shift_changer_backend`, etc.), not just this one. After `wsl --shutdown`, delete the directory, then also prune the two named volumes it left behind (`docker volume rm admin-rebuild-guard_backend_storage admin-rebuild-guard_mysql_data` — confirm nothing needs their data first; they're leftover per-worktree volumes, not the shared `bandms_mysql_data`).
